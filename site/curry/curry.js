...
Elm.CurryGenerator = Elm.CurryGenerator || {};
Elm.CurryGenerator.make = function (_elm) {
   "use strict";
   _elm.CurryGenerator = _elm.CurryGenerator || {};
   if (_elm.CurryGenerator.values) return _elm.CurryGenerator.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var recipeContainerStyle = $Html$Attributes.style(_U.list([]));
   var topStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "margin-bottom",_1: "10px"},{ctor: "_Tuple2",_0: "width",_1: "100%"}]));
   var recipeStorageStyle = $Html$Attributes.style(_U.list([]));
   var currentRecipeStyle = $Html$Attributes.style(_U.list([]));
   var buttonStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "background",_1: "#f9a95e"}
                                                    ,{ctor: "_Tuple2",_0: "margin-right",_1: "3px"}
                                                    ,{ctor: "_Tuple2",_0: "border-radius",_1: "6px"}
                                                    ,{ctor: "_Tuple2",_0: "font-size",_1: "16px"}]));
   var recipeStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: "100%"}
                                                    ,{ctor: "_Tuple2",_0: "text-align",_1: "left"}
                                                    ,{ctor: "_Tuple2",_0: "margin-bottom",_1: "15px"}]));
   var generalStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "font-size",_1: "16px"}
                                                     ,{ctor: "_Tuple2",_0: "font-family",_1: "monospace"}
                                                     ,{ctor: "_Tuple2",_0: "width",_1: "700px"}]));
   var pick = F3(function (input,count,seed) {
      var _p0 = A2($Random$Array.shuffle,seed,input);
      var shuffledArray = _p0._0;
      var newSeed = _p0._1;
      var result = A2($List.take,count,$Array.toList(shuffledArray));
      var prettyResult = A2($String.join,", ",result);
      return {ctor: "_Tuple2",_0: prettyResult,_1: newSeed};
   });
   var buildRecipeStorage = F2(function (recipe,storage) {    return _U.eq(recipe.base,"") ? storage : A2($List._op["::"],recipe,storage);});
   var Store = {ctor: "Store"};
   var Generate = {ctor: "Generate"};
   var newDate = Elm.Native.Port.make(_elm).inbound("newDate",
   "Int",
   function (v) {
   ...
   var recipeView = function (recipe) {
      return A2($Html.div,
      _U.list([recipeStyle]),
      _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"This will be you watery base: ",recipe.base))]))
              ,A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"You can use these spices: ",recipe.spices))]))
              ,A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"And add this as your main ingredient: ",recipe.mainIngredient))]))]));
   };
   var initialRecipe = {base: "",spices: "",mainIngredient: ""};
   var myModel = {recipe: initialRecipe,storedRecipes: _U.list([]),seed: $Random.initialSeed(newDate)};
   var storeRecipe = function (_p1) {
      var _p2 = _p1;
      return {recipe: initialRecipe,storedRecipes: A2(buildRecipeStorage,_p2.recipe,_p2.storedRecipes),seed: _p2.seed};
   };
   var Model = F3(function (a,b,c) {    return {recipe: a,storedRecipes: b,seed: c};});
   var Recipe = F3(function (a,b,c) {    return {base: a,spices: b,mainIngredient: c};});
   var explanation = A2($Html.ul,
   _U.list([]),
   _U.list([A2($Html.li,_U.list([]),_U.list([$Html.text("Roast your main ingredient to your liking and put it aside")]))
           ,A2($Html.li,_U.list([]),_U.list([$Html.text("Then roast the spices for a short while and deglaze them with your watery base")]))
           ,A2($Html.li,_U.list([]),_U.list([$Html.text("After cooking for a short while add your main ingredient and cook until it is tender")]))
           ,A2($Html.li,_U.list([]),_U.list([$Html.text("Serve with rice or bread")]))]));
   var myView = F2(function (address,_p3) {
      var _p4 = _p3;
      return A2($Html.div,
      ...
      _U.list([A2($Html.div,
              ...
              _U.list([A2($Html.h3,
                      _U.list([]),
                      _U.list([$Html.text("Curry-Generator ")
                              ,A2($Html.a,_U.list([$Html$Attributes.href("https://github.com/MrMovl/curry-generator")]),_U.list([$Html.text("(source)")]))]))
                      ,explanation
                      ,A2($Html.hr,_U.list([]),_U.list([]))
                      ,A2($Html.div,
                      _U.list([]),
                      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,Generate),buttonStyle]),_U.list([$Html.text("Generate recipe")]))
                              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Store),buttonStyle]),_U.list([$Html.text("Store recipe")]))]))]))
              ,A2($Html.div,
              _U.list([recipeContainerStyle]),
              _U.list([A2($Html.div,_U.list([currentRecipeStyle]),_U.list([recipeView(_p4.recipe)]))
                      ,A2($Html.hr,_U.list([]),_U.list([]))
                      ,A2($Html.div,_U.list([recipeStorageStyle]),A2($List.map,recipeView,_p4.storedRecipes))]))]));
   });
   var mainIngredient = $Array.fromList(_U.list(["chicken","lamb","cauliflower","aubergine"]));
   var spices = $Array.fromList(_U.list(["pepper","Garam Masala","cumin","cardamom","mustard seeds","cinnamon","clove","chilli","ginger"]));
   var base = $Array.fromList(_U.list(["deglazed onions","coconut milk","tomatoes","joghurt and creme"]));
   var createRandomRecipe = function (_p5) {
      var _p6 = _p5;
      var _p7 = A3(pick,base,1,_p6.seed);
      var newBase = _p7._0;
      var seedOne = _p7._1;
      var _p8 = A3(pick,spices,3,seedOne);
      var newSpices = _p8._0;
      var seedTwo = _p8._1;
      var _p9 = A3(pick,mainIngredient,1,seedTwo);
      var newMainIngredient = _p9._0;
      var seedThree = _p9._1;
      var newRecipe = {base: newBase,spices: newSpices,mainIngredient: newMainIngredient};
      return {recipe: newRecipe,storedRecipes: _p6.storedRecipes,seed: seedThree};
   };
   var myUpdate = F2(function (action,model) {
      var _p10 = action;
      if (_p10.ctor === "Generate") {
            return createRandomRecipe(model);
         } else {
            return storeRecipe(model);
         }
   });
   var main = $StartApp$Simple.start({model: myModel,view: myView,update: myUpdate});
   return _elm.CurryGenerator.values = {_op: _op
                                       ,main: main
                                       ,base: base
                                       ,spices: spices
                                       ...
                                       ,buildRecipeStorage: buildRecipeStorage
                                       ,createRandomRecipe: createRandomRecipe
                                       ,pick: pick
                                       ,generalStyle: generalStyle
                                       ,recipeStyle: recipeStyle
                                       ,buttonStyle: buttonStyle
                                       ,currentRecipeStyle: currentRecipeStyle
                                       ,recipeStorageStyle: recipeStorageStyle
                                       ,topStyle: topStyle
                                       ,recipeContainerStyle: recipeContainerStyle};
};
